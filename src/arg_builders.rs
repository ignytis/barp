...
use crate::types::process_params::{CommandArgs, ProcessParams};
...
fn build_params_for_command(task_cfg: &ConfigParam, cmd_args: &VecDeque<String>) -> Result<ProcessParams, String> {
    let cfg = serde_yaml::to_string(task_cfg).unwrap();
...
fn build_params_with_lua(arg_builder_name: &String, task_cfg: &ConfigParam, cmd_args: &VecDeque<String>) -> Result<ProcessParams, String> {
    ...
    let mut args = cmd_args.clone();
    args_final.append(&mut args);
    let args_final: Vec<String> = args_final.into();
    ...
    let mut args_final: VecDeque<String> = args_final.into();
